...
def ordinary(data, col, step=1, reverse=False, drop=True):
    """
    将数据中的某一列使用序号编码进行替换，
...
def binary(data, col, reverse=False, drop=True):
    """
    将数据中的某一列使用二进制编码进行替换
...
def one_hot(data, col, drop=True, engine='pd'):
    """
...
def ratio(data, col, n_digits=3, drop=True):
    """
    将数据中的某一列使用其出现频率进行编码
...
def concat_data(drop, data, new_data, col):
